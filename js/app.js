/*! ------------------------------------------------
 * Project Name: Worthy Chukwuemeka - Personal Portfolio
 * Project Description: Worthy Chukwuemeka – Crafting innovative digital experiences. Specializing in web development, CRM systems, and creative solutions to build the future of tech.
 * Version: 1.0.0
 * File name: app.js
 * ------------------------------------------------
  ...
  if (items) {
    ...
    items.forEach((itemBlock) => {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          master.timeScale(timeScaleClamp(Math.abs(self.getVelocity() / 1000)));
...
const logo = document.getElementById('theme-logo'); // Target the logo image by id
...
function loadTheme(theme) {
  ...
  if (theme === 'dark') {
    logo.src = './img/2.png'; // Dark mode logo
  } else {
    logo.src = './img/1.png'; // Light mode logo
  }
}
